...
describe("chat message renderer", () => {
  ...
  it("shows a recoverable error message", () => {
    ...
    expect(
      screen.getByText("The response was incomplete."),
    ).toBeTruthy()
...
describe("chat composer", () => {
  it("validates an empty form and submits a non-empty query", () => {
    ...
    fireEvent.submit(
      input.closest("form") as HTMLFormElement,
    )
  ...
  it("renders available tool output for the user", () => {
    render(
      <ToolState
        ...
        outputContent={
          <p>Found three films for you.</p>
        }
      />,
    )

    expect(
      screen.getByText("Found three films for you."),
    ).toBeTruthy()
  })
})
