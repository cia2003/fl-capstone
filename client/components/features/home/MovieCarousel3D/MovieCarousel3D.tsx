...
import type { CarouselControlValues } from "./CarouselControls";

const DynamicCarouselScene = dynamic(
  () =>
    import("./CarouselScene").then(
      (module) => module.CarouselScene,
    ),
  {
    ssr: false,
    loading: () => null,
...
const DynamicCarouselControls = dynamic(
  () =>
    import("./CarouselControls").then(
      (module) => module.CarouselControls,
    ),
  {
    ssr: false,
  },
...
export function MovieCarousel3D({
  ...
  const updateControl = useCallback(
    <Key extends keyof CarouselControlValues>(
      key: Key,
      value: CarouselControlValues[Key],
    ) => {
      setControls((current) => ({
        ...current,
        [key]: value,
  ...
  useImperativeHandle(
    ...
    () => ({
      goToPrevious: () =>
        setActiveIndex(
          (index) =>
            (index - 1 + topFilms.length) %
            topFilms.length,
        ),

      goToNext: () =>
        setActiveIndex(
          (index) => (index + 1) % topFilms.length,
        ),
  ...
  return (
    <section className="mx-5 md:mx-10 lg:mx-16 min-[1440px]:mx-24">
      ...
      <div className="relative h-[500px] w-full">
        ...
        >
          <DynamicCarouselScene config={sceneConfig} />
        </Canvas>
