...
export type CarouselControlsConfig = {
  activeIndex: number;
  films: Film[];
  values: CarouselControlValues;
  onChange: <Key extends keyof CarouselControlValues>(
    key: Key,
      ...
      {activeFilm && (
        ...
        >
          ...
          <p className="mt-1 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm text-muted-foreground">
            ...
            <span>
              {activeFilm.running_time} min
            </span>

            <span aria-hidden="true">·</span>

            <span>
              Dir. {activeFilm.director}
            </span>
          </p>
        </Link>
      )}
